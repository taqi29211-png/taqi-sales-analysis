...
if st.button("Generate AI Analysis"):
    ...
    # Create a summary of the current dashboard data
    ai_summary = {
        ...
        "Top Salesperson Revenue": float(
            top_salesperson_revenue
        ),

        "Most Sold Product": str(
            most_sold_product
        ),

        "Most Sold Quantity": int(
            most_sold_quantity
        ),

        "Best Sales Day": best_sales_day.strftime(
            "%d %b %Y"
        ),

        "Best Sales Day Revenue": float(
            best_sales_day_revenue
        )
    }
...
Analyze the following sales dashboard data:

{ai_summary}

Provide a clear business analysis containing:

1. Overall performance
2. Strongest areas
3. Areas that may need attention
4. Three useful observations
...
if len(monthly_sales) >= 2:
    ...
    if latest_growth > 0:
        ...
        st.success(
            f"📈 Revenue increased by {latest_growth:.2f}% "
        ...
        st.warning(
            f"📉 Revenue decreased by {abs(latest_growth):.2f}% "
            "compared with the previous month."
        )
        ...
        st.info(
            "➡️ Revenue remained unchanged compared with "
            "the previous month."
        )
    ...
    st.info(
        "ℹ️ More than one month of sales data is needed "
        "to calculate growth."
    )
